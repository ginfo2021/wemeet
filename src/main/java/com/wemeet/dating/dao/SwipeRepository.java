...
public interface SwipeRepository extends BaseRepository<Swipe, Long> {
    ...
    @Query(
            value
                    ...
                    " AND  A.swiper_id = :userId AND A.TYPE = :swipeType AND B.TYPE = :swipeType",
            countQuery
                    ...
                    " WHERE A.swiper_id = B.swipee_id " +
                    " AND B.swiper_id = A.swipee_id " +
                    " AND  A.swiper_id = :userId AND A.TYPE = :swipeType AND B.TYPE = :swipeType) t",
            nativeQuery = true)
    ...
    @Query(
            value
                    ...
                    " FROM user U  " +
                    " INNER JOIN swipe SW " +
                    " ON U.id = SW.swiper_id " +
                    " WHERE  " +
                    " U.deleted = 0  " +
                    " AND U.active = 1  " +
                    " AND U.suspended = 0  " +
                    " AND SW.swipee_id = :userId " +
                    " AND :userId NOT IN (SELECT swiper_id FROM swipe WHERE swipee_id = U.id)  ",
            nativeQuery = true
    )
    List<BigInteger> findAdmirers(Long userId);
    ...
    List<BigInteger> findAdmirersWithPreferences(Long userId, List<String> genderPreferenceList, int limit);
